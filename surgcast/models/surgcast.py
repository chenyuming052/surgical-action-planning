...
from .heads import MultiTaskHeads, MultiTaskHeadsV2
from .hazard_head import DualHazardHead, PhaseGatedDualHazardHead, StateAgeEncoder
...
class SurgCastModelV2(nn.Module):
    """Full Version B pipeline — event-conditioned SurgCast.

    Wires: CausalTemporalTransformer -> ActionTokenEncoder -> NextActionHead
           -> PhaseGatedDualHazardHead -> EventDyn -> MultiTaskHeadsV2
    ...
    def __init__(
        ...
        self.hazard_head = PhaseGatedDualHazardHead(
            hidden_dim=hidden_dim,
            action_dim=action_dim,
            trunk_dim=hazard_trunk_dim,
            num_bins=hazard_num_bins,
    ...
    def forward(
        ...
        anatomy_logits_for_sg = None  # will be computed inside heads if needed
        ...
        out.update({f"next_{k}": v for k, v in next_out.items()})
