...
**版本：v5.3 (Reviewer Feedback Integration)**
**目标会议：NeurIPS 2026**
**后续拓展：TMI / Nature 子刊**
...
[Module B] Causal Temporal Transformer (6 layers, 8 heads, dim=512)
        ...
        │         │
        │         └──→ h(k | h_t) = P(T=k | T≥k, h_t), k=1,...,K (K=20, 非均匀 bins)
        │
        └──→ [Module E] Structured Prior Regularization (核心创新)
                  │
                  ├──→ Evidence-gating: KL 权重按 cell count / posterior entropy 自适应调节
                  └──→ L_prior = w_evidence · KL(p_θ(y_miss | h_t) ‖ q_prior(y_miss | y_obs, h_t))
```
...
- Head 结构：Linear(512, 5)，per-class sigmoid，BCE loss
- **仅在有 bbox 标注的视频上激活 loss**：G1(3) + G3(3) + G4(3) + G7(192) = 201 个视频
...
**[v5.3 修正] 非均匀时间离散化：** 将未来时间离散化为 K=20 个非均匀 bin，覆盖未来 30 秒：

```
Bins: [1s, 2s, 3s, 4s, 5s, 6s, 7s, 8s, 9s, 10s, 12s, 14s, 16s, 18s, 20s, 22s, 24s, 26s, 28s, 30s]
```

前 10 个 bin 逐秒（1-10 秒，精细分辨率），后 10 个 bin 每 2 秒（12-30 秒，粗分辨率）。
...
其中 f_hazard 是一个 MLP：Linear(516, 256) → GELU → Linear(256, K=20)，σ 是 sigmoid。输入为 [h_t; σ_agg]（512 维状态 + 4 维转移不确定性），σ_agg 来自 Module C 的 heteroscedastic 输出（见 4.4）。
...
**[v5.3] 非均匀 bins 的 TTC 计算：** TTC 期望值 = Σ_k t_k · P(T = t_k | h_t)，其中 t_k 是第 k 个 bin 的中心时间点。对非均匀 bins，每个 bin 的宽度不同（前 10 个宽度 1s，后 10 个宽度 2s），TTC MAE 和 Brier score 的计算需相应调整。
...
**Regularization loss（evidence-gated）：**

```
L_prior = w_evidence(y_obs) · D_KL(p_θ(y_miss | h_t) ‖ q_prior(y_miss | y_obs, h_t))
```
...
L_task = m_tri · L_triplet + m_inst · L_instrument + m_pha · L_phase + m_cvs · L_cvs + m_anat · L_anatomy
    ...
    (L_triplet、L_instrument、L_anatomy 为 BCE loss；L_cvs 为 ordinal BCE loss；L_phase 为 CE loss)
    (v5.0 修正：L_triplet 从 CE 改为 BCE，因 triplet-group 为 multi-hot 多标签预测)
    ([v5.3] L_anatomy 仅在有 bbox 标注的视频上激活：G1/G3/G4/G7)

L_align = Σ_Δ ‖ĥ_{t+Δ} - sg(h_{t+Δ})‖²
    (Latent transition alignment, Δ ∈ {1, 3, 5, 10})

L_hazard = 离散时间生存损失（见 4.5.3，K=20 非均匀 bins）

L_prior = w_evidence · D_KL(p_θ(y_miss | h_t) ‖ q_prior(y_miss | y_obs, h_t))
...
| Hazard head | ~138K | **[v5.3] 输入 516→256→20（4-d σ_agg, K=20 bins）** |
...
加上冻结的 DINOv3 86M 参数，推理时总参数 ~97M，但只有 ~11.2M 需要训练。在 2×A100 40GB 上训练毫无压力。v5.3 新增的 anatomy head / source calibration / evidence-gating / Δ=10 embedding 总计不到 3K 参数，几乎不影响模型规模，但显著改善了评估覆盖（anatomy）、跨源泛化（CVS calibration）和 prior 可信度（evidence-gating）。
...
- **[v5.3] 激活 L_anatomy 的样本：G1 + G3 + G4 + G7 = 201 个视频（73%）**
...
|---|---|---:|---|---|
| **Tier 1: Core** | CholecT50-test (G2+G3+G5) | ~10 | Change-mAP (instrument-set + triplet-group), TTC 全组, Future-state Acc, Dense-mAP, Safety B1 | **主表 (Table 1) 的评估基准** |
| **Tier 2a: CVS Safety (主)** | **Endoscapes-test (G7) + Cholec80-test (G6)** | **~35** | **CVS criterion AUC, CVS MAE** | **[v5.3] 主 CVS 评估集，统计基础更强** |
| **Tier 2b: CVS-at-Clipping (临床 stress test)** | **Tier 1 中有 CVS 的 (G2)** | **~8** | **CVS MAE@clip, Early Warning Quality** | **[v5.3] 临床关键窗口补充证据** |
| **Tier 3: Phase** | Cholec80-test (G2+G6) | ~13 | Phase Acc @Δ, **Phase-level TTC** | 扩大评估基数，验证粗粒度泛化 |
| **Tier 4: Instrument** | CholecT50-test + Cholec80-test (G2+G3+G5+G6) | ~15 | Instrument Anticipation mAP, **Instrument-set Change-mAP** | **[v5.3] 新增 instrument-set change 评估** |
| **Tier 5: Anatomy** | **Endoscapes-test (G7)** | **~30** | **Anatomy-Presence mAP** | **[v5.3 新增] 验证 bbox → anatomy presence 的学习效果** |
...
1. 报告 **per-video** Change-mAP 和 TTC C-index（不仅是均值，展示分布）
2. 报告 5 次随机种子的均值 ± std
...
#### 7.0.5 Phase-Level Change 辅助评估

**[v5.2 新增] 在 Cholec80-test 上构造 phase change point，评估粗粒度 TTC。**
...
| Phase-TTC C-index | Phase change 的 concordance index |
| Phase-TTC Brier @5s | Phase change 的概率校准 |
...
**Hazard head 适配：** Phase-TTC 不需要额外的 hazard head。在推理时，将 triplet-level change 聚合到 phase-level：如果模型预测未来 k 秒内 phase 会变（通过 phase head 输出在不同 horizon 上的变化），直接从 hazard head 的 survival function 中提取。具体实现在 Phase 1 完成后确认。

**放置位置：** Table 1 增加一列 Phase-TTC C-index，或单独一个小表放 appendix。
...
| **TTC MAE** | time-to-next-change 的平均绝对误差（秒），分别报告 instrument-level 和 group-level | TTC 预测精度 |
| **TTC C-index** | concordance index，衡量 TTC 排序一致性 | 区分力（是否能正确区分"快变"和"慢变"） |
| **TTC Brier Score** | 对"未来 k 秒内是否发生 change"的概率预测的 Brier score，k ∈ {5, 10, 20} | 概率校准质量（**[v5.3] k 范围扩展至 20s 以匹配 K=20 bins**） |
...
- 两个 baseline 的复现策略：优先使用原文开源代码适配到我们的数据 protocol；如无开源代码，按原文方法描述在我们的架构上复现核心模块
...
**评估范围：** Change-mAP / TTC 组 / Dense-mAP 在 **Tier 1 测试集**（~10 CholecT50-test 视频）上评估；Phase Acc 在 **Tier 3 测试集**（~13 Cholec80-test 视频）上评估。所有方法的测试集完全固定，仅训练数据/方法不同（见 7.0.6）。报告 5 seeds 的 mean ± std。

| Method | Inst-C-mAP | Group-C-mAP | TTC MAE ↓ | TTC C-index ↑ | Brier @5s ↓ | Dense-mAP | Phase Acc |
|---|---|---|---|---|---|---|---|
...
主文放 A1-A5（structured prior 相关）、**A_evidence（evidence-gating，关键消融）**、A6'（transition 参数共享）、**A_new（不确定性→hazard 信号，最关键消融）**、A8-A9（hazard 相关）、A10a（domain-specific backbone）、A16（CVS 来源）、A18（CVS ordinal vs binary）。A_σgate 视实现情况决定放主文或 appendix。**A_src、A_anat、A_K、A_xval 放 appendix。** 其余放 appendix。
...
完整架构图。左：冻结 DINOv3 ViT-B/16 + Causal Transformer。中：**Shared Transition MLP + Horizon Embedding（Δ={1,3,5,10}）**（突出参数共享和 σ 输出分支）+ Multi-horizon heads（突出 Instrument Head + **Anatomy-Presence Head [v5.3]** + **source-calibrated CVS Head [v5.3]**）。右上：Discrete-Time Hazard Head（**标注 σ_agg ∈ R⁴ 输入，K=20 非均匀 bins [v5.3]**）。右下：Structured Prior Regularization（static prior 查表 + context modulation + **evidence-gating [v5.3]**），展示分解条件分布。
...
2. 我们定义了 action-change anticipation：预测下一次动作变化何时发生（discrete-time hazard TTC）、变化后的动作集合是什么（multi-label post-change state）、以及变化是否安全（CVS state at critical transition）——并提出了三层 event-centric benchmark（instrument-set / triplet-group / clipping event），消除对单一聚类定义的依赖。
...
| 输出 | 每个视频一个 `.npz`：frames, cvs_scores, in_roi, **anatomy_presence(5维)** |
...
| Collate function | 输出：features, labels（含 instrument/verb）, visibility_masks, ttc_targets, censoring_flags, cvs_targets, cvs_source_ids, anatomy_presence, is_clipping **[v5.3 新增 cvs_source_ids, anatomy_presence]** |
...
| **外部 baseline 无法复现** | 中 | 中 | **[v5.3 新增] 优先使用原文开源代码；如无开源则按方法描述在我们架构上复现核心模块；最差情况下用 ablation baseline 代替并在论文中注明** |
